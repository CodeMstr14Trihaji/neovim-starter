...
local function load_state()
  ...
  if not fd then return { last = "", count = 0, reminders = {} } end
  local stat = uv.fs_fstat(fd)
  if stat.size == 0 then uv.fs_close(fd); return { last = "", count = 0, reminders = {} } end
...
local function days_between(date1, date2)
  ...
  if not (y1 and m1 and d1 and y2 and m2 and d2) then return 0 end
